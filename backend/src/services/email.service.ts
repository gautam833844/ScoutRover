...
class EmailService {
    ...
    try {
      if (this.transporter) {
        await this.transporter.sendMail({
          from: `"Atlas Support" <support@atlas.local>`,
          to,
          subject,
          text,
          html,
  ...
  async sendWelcomeEmail(to: string, name: string): Promise<boolean> {
    const subject = 'Welcome to Atlas!';
    const text = `Hi ${name},\n\nWelcome to Atlas Autonomous Mapping Dashboard!\nYour user account has been successfully created.\n\nBest regards,\nThe Atlas Team`;
    const html = `
      <div style="font-family: Arial, sans-serif; padding: 20px; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px;">
        <h2 style="color: #7c3aed;">Welcome to Atlas, ${name}!</h2>
        ...
        <p>Best regards,<br/><strong>The Atlas Team</strong></p>
      </div>
    `;
  ...
  async sendPasswordResetEmail(to: string, name: string, token: string): Promise<boolean> {
    ...
    const subject = 'Atlas Password Reset Request';
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; padding: 20px; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px;">
        ...
        <p>Hi ${name},</p>
        <p>You received this email because a password reset request was submitted for your Atlas account. Click the button below to set a new password:</p>
        <div style="margin: 25px 0;">
